...
from apps.registration.models import Trip, TripStage, StayOrganizer, StayOrganizerType, ClientData,TripWarning
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Create Trip warning for Poland and Gemany
        trip_warning, _ = TripWarning.objects.get_or_create(
            content= "Sesja się zbliża!",
            country = poland
        )

        trip_warning2, _ = TripWarning.objects.get_or_create(
            content="Germany is under attack of Kongo!",
            country=germany
        )
